...
def test_canonical_schemas_remain_v110_and_repository_schemas_are_versioned():
    ...
    assert schema['x-global-impact-catalyst-version']=='2.0.0'
    assert '/2.0.0/' in schema['$id']
    schema=load('global_impact_production_repository.schema.json')
    assert schema['x-global-impact-catalyst-version']=='1.10.0'
    assert '/1.10.0/' in schema['$id']
    schema=load('global_impact_platform_repository.schema.json')
    assert schema['x-global-impact-catalyst-version']=='2.0.0'
    assert '/2.0.0/' in schema['$id']
    ...
    reporting=load('global_impact_reporting_repository.schema.json')
    assert reporting['x-global-impact-catalyst-version']=='1.8.0'
    assert '/1.8.0/' in reporting['$id']
