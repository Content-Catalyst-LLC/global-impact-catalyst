<?php
// Minimal WordPress stubs for release-contract rendering checks.
...
if (count($ids) !== count(array_unique($ids))) {
    fwrite(STDERR, "Duplicate shortcode IDs detected.\n");
    exit(1);
}
preg_match_all('/\sfor="([^"]+)"/', $html, $label_matches);
foreach ($label_matches[1] as $target) {
    if (!in_array($target, $ids, true)) {
        fwrite(STDERR, "Label target is missing: {$target}\n");
        exit(1);
    }
}
if (substr_count($html, 'data-gic-demo') !== 2) {
    fwrite(STDERR, "Expected two shortcode instances.\n");
    exit(1);
}
echo "WordPress multi-instance contract passed for " . count($ids) . " unique IDs.\n";
